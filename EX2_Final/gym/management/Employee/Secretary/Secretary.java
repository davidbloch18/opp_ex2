...
public class Secretary {
    ...
    public void notify(Object targetType, String message) {
        if (targetType instanceof Session) {
            ...
            try{
                String formattedDate = new SimpleDateFormat("yyyy-MM-dd").format(new SimpleDateFormat("dd-MM-yyyy").parse(date));
                NotificationCenter.notifySessionObservers(message, date);
                NotificationCenter.logAction(String
                        .format("A message was sent to everyone registered for a session on %s : %s", formattedDate, message));
            }
